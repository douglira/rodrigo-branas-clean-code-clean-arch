...
export class StoreRepository implements StoreRepositoryInterface {
  ...
  private factoryStore(result: any): Store {
    return new Store(
      result.id,
      result.name,
      new Address(
  ...
  async get(storeId: string): Promise<Store> {
    ...
    if (result) {
      return this.factoryStore(result);
    }
  }
  async getNearby(coord: Coordinates): Promise<Store> {
    const result = await this.storeDatabase.getNearby(coord.lat, coord.lng);
    if (result) {
      return this.factoryStore(result);
    }
  }
